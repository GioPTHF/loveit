<?php
	if(isset($_POST['namefunction'])){
		...
		switch ($namefunction) {
			...
			case 'modalContactanosPropiedad':
				...
				break;
			case 'eliminarSubcategoriaInmobiliaria':
				eliminarSubcategoriaInmobiliaria();
				break;
	...
	function eliminarSubcategoriaInmobiliaria(){
		if ($_POST['idInmobiliaria'] == 1) {
			$query = "SELECT * FROM subcategoriaInmobiliaria sub
						INNER JOIN Proyectos p ON p.subcategoriaInmobiliaria_idsubcategoriaInmobiliaria = sub.idsubcategoriaInmobiliaria
						WHERE sub.idsubcategoriaInmobiliaria = '".$_POST['idTipoSubInmobiliaria']."'";
			$result = mysql_query($query,Conectar::con()) or die(mysql_error());
			$num_row = mysql_num_rows($result);
			if ($num_row == 0) {
				$query = "DELETE FROM subcategoriaInmobiliaria WHERE idsubcategoriaInmobiliaria = '".$_POST['idTipoSubInmobiliaria']."'";
				$result = mysql_query($query,Conectar::con()) or die(mysql_error());
				echo 1;
			} else {
				echo -1;
			}
		} else if ($_POST['idInmobiliaria'] == 2) {
			$query = "SELECT * FROM subcategoriaInmobiliaria sub
						INNER JOIN Desarrollos d ON d.subcategoriaInmobiliaria_idsubcategoriaInmobiliaria = sub.idsubcategoriaInmobiliaria
						WHERE sub.idsubcategoriaInmobiliaria = '".$_POST['idTipoSubInmobiliaria']."'";
			$result = mysql_query($query,Conectar::con()) or die(mysql_error());
			$num_row = mysql_num_rows($result);
			if ($num_row == 0) {
		...
		} else if ($_POST['idInmobiliaria'] == 3) {
			$query = "SELECT * FROM subcategoriaInmobiliaria sub
						INNER JOIN Propiedades pro ON pro.subcategoriaInmobiliaria_idsubcategoriaInmobiliaria = sub.idsubcategoriaInmobiliaria
						WHERE sub.idsubcategoriaInmobiliaria = '".$_POST['idTipoSubInmobiliaria']."'";
			$result = mysql_query($query,Conectar::con()) or die(mysql_error());
			$num_row = mysql_num_rows($result);
			if ($num_row == 0) {
